...
// Helpers
Expr exec_int_binary_operation(pExecutor exec, Expr a, Expr b, pLongInt (*op)(pLongInt, pLongInt), char *caller)
{
    ...
    pLongInt num = op(dereference(a).val_int, dereference(b).val_int);
...
long exec_int_to_long(pExecutor exec, Expr num)
{
    ...
    return longint_to_long(dereference(num).val_int);
}
double exec_int_to_double(pExecutor exec, Expr num)
{
    ...
    return longint_to_double(dereference(num).val_int);
}

Expr exec_int_add(pExecutor exec, Expr a, Expr b)
{
    return exec_int_binary_operation(exec, a, b, longint_sum, "exec_int_add");
}
Expr exec_int_sub(pExecutor exec, Expr a, Expr b)
{
    return exec_int_binary_operation(exec, a, b, longint_difference, "exec_int_sub");
}
Expr exec_int_inverse(pExecutor exec, Expr a)
{
    ...
    pLongInt num = longint_inverse(dereference(a).val_int);
    if (num == NULL)
    {
        log("exec_int_inverse: longint_inverse failed");
        exit(1);
    }
    Expr res = make_int(exec, num);
    free_longint(num);
...
Expr exec_int_inc(pExecutor exec, Expr a)
{
    ...
    pLongInt one = longint_one();
    if (one == NULL)
    {
        log("exec_int_inc: longint_one failed");
        exit(1);
    }
    pLongInt num = longint_sum(dereference(a).val_int, one);
    if (num == NULL)
    {
        log("exec_int_inc: longint_sum failed");
        exit(1);
    }
    free_longint(one);
    Expr res = make_int(exec, num);
...
{
    return exec_int_binary_operation(exec, a, b, longint_product, "exec_int_product");
}
Expr exec_int_div(pExecutor exec, Expr a, Expr b)
{
    return exec_int_binary_operation(exec, a, b, longint_quotient, "exec_int_div");
}
Expr exec_int_rem(pExecutor exec, Expr a, Expr b)
{
    return exec_int_binary_operation(exec, a, b, longint_remainder, "exec_int_rem");
}
Expr exec_int_division_inverse(pExecutor exec, Expr a)
{
    ...
    if (one == NULL)
    {
        log("exec_int_division_inverse: longint_one failed");
        exit(1);
    }
    pLongInt num = longint_quotient(one, dereference(a).val_int);
    if (num == NULL)
    {
        log("exec_int_division_inverse: longint_quotient failed");
        exit(1);
    }
    free_longint(one);
    Expr res = make_int(exec, num);
...
{
    return exec_int_binary_operation(exec, a, b, longint_gcd, "exec_int_gcd");
}
...
Expr exec_convert_to_real(pExecutor exec, Expr expr)
{
    if (is_real(expr))
        ...
        return make_real(exec, longint_to_double(dereference(expr).val_int));
...
Expr exec_convert_to_int(pExecutor exec, Expr expr)
{
    if (is_real(expr))
    {
        pLongInt num = longint_from_double(expr.val_real);
        Expr res = make_int(exec, num);
        free_longint(num);
        return res;
    }
    ...
    {
        pLongInt num = longint_from_long(expr.val_char);
        Expr res = make_int(exec, num);
        free_longint(num);
        return res;
    }
...
Expr exec_convert_to_char(pExecutor exec, Expr expr)
{
    if (is_real(expr))
        ...
        return make_char(exec, (char) longint_to_long(dereference(expr).val_int));
    else if (is_char(expr))
        return expr;
    else
        return make_none();
...
{
    return longint_compare(dereference(a).val_int, dereference(b).val_int);
}
...
Expr exec_division_inverse(pExecutor exec, Expr a)
{
    ...
    {
    case VT_CHAR:
        return make_char(exec, (char) (1 / a.val_char));
    case VT_INT_PTR:
